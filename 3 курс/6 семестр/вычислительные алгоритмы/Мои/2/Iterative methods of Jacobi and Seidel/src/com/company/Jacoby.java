...
public class Jacoby {
    ...
    public static void printTabel(PrintWriter pw, int a){
        if (a == 1) {
            ...
            System.out.println("|    i |            x1 |            x2 |            x3 |      eps  |          eps1 |          eps2 |         eps3  |");
            System.out.println("--------------------------------------------------------------------------------------------------------------------");
            ...
            pw.println("|    i |            x1 |            x2 |            x3 |      eps  |          eps1 |          eps2 |         eps3  |");
            pw.println("--------------------------------------------------------------------------------------------------------------------");
        }
        if (a == 2) {
            System.out.println("--------------------------------------------------------------------------------------------------------------------");
    ...
    public void method() throws FileNotFoundException {
        ...
        boolean view = false;
        File inputFile = new File("input.txt");//файл для чтения
        PrintWriter pw = new PrintWriter("output.txt");//файл для записи
        ...
        do{ // Пока погрешность выше eps
            ...
            for (int i = 0; i < n; i++) {
                epsil[i] = Math.abs(x[i] - x0[i]) / Math.abs(x[i]);
                x0[i] = x[i];
                if (view) System.out.printf("%14f |", x0[i]);
                if (view) pw.printf("%14f |", x0[i]);
            }
            counter++;
            for (int i = 0; i < n; i++) {
                if (view) System.out.printf("%14f |", epsil[i]);
                if (view) pw.printf("%14f |", epsil[i]);
            }
