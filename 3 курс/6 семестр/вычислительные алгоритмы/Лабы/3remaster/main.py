...
def lagrange_interpolation(x, y, x_interp):
 """
 Вычисляет интерполяционный многочлен Лагранжа.
 Аргументы:
 x -- массив узлов интерполяции
 y -- массив значений функции в узлах интерполяции
 x_interp -- точка, в которой требуется вычислить значение интерполяционного
многочлена
 Возвращает:
...
def plot_graphs(x, y, x_interp, y_interp, x_intersect, y_intersect):
 ...
 # Определяем максимальное отклонение
    max_err = max_error(y_interp, y)
    print('Максимальное отклонение: ', max_err)
 # Добавляем легенду и выводим графики на экран
    plt.legend()
    plt.show()
def max_error(y1, y2):
 """
